...
class Cl_User
{
	...
	public function login( array $data )
	{
		...
		if( !empty( $data ) ){
			...
			$password = md5( $password );
			$query = "SELECT user_id, name, email, social_id, picture, created FROM users where email = '$email' and password = '$password' ";
	...
	public function google_login( array $data )
	{
		...
		$query = "SELECT user_id, name, email, social_id, picture, created FROM users where email = '$email' and social_id = '$social_id' ";
		$result = mysqli_query($this->_con, $query);
		$data = mysqli_fetch_assoc($result);
		$count = mysqli_num_rows($result);
		if( $count == 1){
	...
	public function twitter_login( array $data )
	{
		...
		if( $count == 1){
			...
			if(mysqli_query($this->_con, $query)){
				...
				$headers = "From: admin@exclaimadeasy.com" . "\r\n" .
						"CC: shashank.ssriv@gmail.com";
